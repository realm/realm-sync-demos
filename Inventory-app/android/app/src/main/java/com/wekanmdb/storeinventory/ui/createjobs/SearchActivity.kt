...
class SearchActivity : BaseActivity<ActivitySearchBinding>() {
    ...
    private fun initStoreAdapter() {
        ...
        val sourceStoreId = user?.customData?.getObjectId("stores")
    ...
    private fun listenStoreAdapter(searchedItem: String) {
        val stores =
            apprealm?.where<Stores>()?.contains("name", searchedItem,Case.INSENSITIVE)?.findAllAsync()
    ...
    fun updateProduct(id: String, name: String?, totalQuantity: Int?, ) {
        val output = Intent()
